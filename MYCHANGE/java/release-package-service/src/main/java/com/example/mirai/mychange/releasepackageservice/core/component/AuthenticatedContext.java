...
public class AuthenticatedContext implements RBACInitializerInterface, AuditableUserExtractorInterface, ApplicationEventActorExtractorInterface, EventActorExtractorInterface {
    ...
    public User getAuditableUser() {
        ...
        //added this null check as in automatic closure, it is calling getCurrentAuditor and newRevision method one after the other
        // the first called method removing audit user from thread local and so second method is not finding user details in thread local
        // and audit records are saved with user id null
