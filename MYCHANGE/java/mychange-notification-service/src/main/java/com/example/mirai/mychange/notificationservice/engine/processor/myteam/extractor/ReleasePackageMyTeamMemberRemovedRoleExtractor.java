...
@Component("ReleasePackageMyTeamMemberRemoved")
public class ReleasePackageMyTeamMemberRemovedRoleExtractor extends MyTeamMemberRoleExtractor {

    public Set<BaseRole> getProcessors(Event event) throws IOException {
        Set<BaseRole> baseRoles = super.getProcessors(event);
        baseRoles.add(new MyTeamMemberRole(event, "MyTeamMember", myTeamMemberUser, "ReleasePackageMyTeamMemberRemoved", myTeamMemberId, getEntityId(event)));
        return baseRoles;
    }
}
