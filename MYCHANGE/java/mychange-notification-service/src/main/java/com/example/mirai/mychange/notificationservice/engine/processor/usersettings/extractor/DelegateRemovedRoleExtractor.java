...
import com.example.mirai.projectname.notificationservice.engine.processor.usersettings.role.DelegateRole;
import com.example.mirai.projectname.notificationservice.engine.processor.usersettings.role.DelegatorRole;
...
//@Component("DelegateRemoved")
public class DelegateRemovedRoleExtractor implements RoleExtractorInterface {
	...
	public Set<BaseRole> getProcessors(Event event) throws IOException {
		...
		data.stream().forEach(item -> {
			...
			baseRoles.add(new DelegateRole(event, "DelegateRole", delegate, "DelegateRemoved", null, null));
		});
		return baseRoles;
	}
}
