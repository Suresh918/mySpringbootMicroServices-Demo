...
public class ReviewCreatedRoleExtractor implements RoleExtractorInterface {
	@Override
	public Set<BaseRole> getProcessors(Event event) throws IOException {
		...
		baseRoles.add(new ReviewExecutorRole(event, "ReviewExecutor", executor, "ReviewCreated", entityId, entityId));
		...
		if (reviewTasks != null) {
			for (Map reviewTaskProperties : reviewTasks) {
				...
				baseRoles.add(new ReviewTaskAssigneeRole(event, "ReviewTaskAssignee", reviewTaskAssignee, "ReviewCreated", Long.parseLong(id.toString()), entityId));
			}
		}
		return baseRoles;
	}
