...
public class ReviewCompletedRoleExtractor implements RoleExtractorInterface {
	...
	public Set<BaseRole> getProcessors(Event event) throws IOException {
		...
		baseRoles.add(new ReviewExecutorRole(event, "ReviewExecutor", executor, "ReviewCompleted", entityId, entityId));
		...
		for (Map reviewTaskProperties : reviewTasks) {
			...
			if (recipients.stream().filter(user -> user.getUserId().equals(reviewTaskAssignee.getUserId())).findFirst().isEmpty()) {
				baseRoles.add(new ReviewTaskAssigneeRole(event, "ReviewTaskAssignee", reviewTaskAssignee, "ReviewCompleted", Long.parseLong(id.toString()), entityId));
		...
		for (Map reviewEntryProperties : reviewEntries) {
			...
			if (recipients.stream().filter(user -> user.getUserId().equals(reviewEntryAssignee.getUserId())).findFirst().isEmpty()) {
				baseRoles.add(new ReviewEntryAssigneeRole(event, "ReviewEntryAssignee", reviewEntryAssignee, "ReviewCompleted", Long.parseLong(id.toString()), entityId));
