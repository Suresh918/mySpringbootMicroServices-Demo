...
@Component("ChangeRequestMyTeamMemberRemoved")
public class ChangeRequestMyTeamMemberRemovedRoleExtractor extends MyTeamMemberRoleExtractor {

    public Set<BaseRole> getProcessors(Event event) throws IOException {
        Set<BaseRole> baseRoles = super.getProcessors(event);
        baseRoles.add(new MyTeamMemberRole(event, "MyTeamMember", myTeamMemberUser, "ChangeRequestMyTeamMemberRemoved", myTeamMemberId, getEntityId(event)));
        return baseRoles;
    }
}
