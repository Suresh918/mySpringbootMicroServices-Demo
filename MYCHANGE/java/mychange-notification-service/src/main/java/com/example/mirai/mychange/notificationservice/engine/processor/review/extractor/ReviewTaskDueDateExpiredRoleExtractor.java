...
@Component("ReviewTaskDueDateExpired")
public class ReviewTaskDueDateExpiredRoleExtractor implements RoleExtractorInterface {
	@Override
	public Set<BaseRole> getProcessors(Event event) throws IOException {
		...
		//there will be exactly 1 reviewTask in reviewTask event
		...
		baseRoles.add(new ReviewTaskAssigneeRole(event, "ReviewTaskAssignee", reviewTaskAssignee, "ReviewTaskDueDateExpired", Long.parseLong(id.toString()), entityId));
		return baseRoles;
	}
}
